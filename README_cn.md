...
## onnx模型转tflite模型 依赖环境+脚本 （待完善）

Python 环境只用于 `tools/verify_tflite.py` 检查模型；编译和运行 C++
唤醒程序不依赖 Conda、NumPy 或 TensorFlow。已验证的 Python 组合为
Python 3.10、NumPy 1.24.0 和 TensorFlow 2.16.2。
...
```bash
./build/bin/kws_main fbank 40 256 examples/audio/silence.wav
./build/bin/kws_main fbank 40 256 examples/audio/0000e12e2402775c2d506d77b6dbb411.wav
./build/bin/kws_main fbank 40 256 /path/to/16k_mono.wav
```
