...
// Modified by GengXin Zheng in 2026 to support MFCC and Fbank selection.
//
...
FeaturePipeline::FeaturePipeline(const FeaturePipelineConfig& config)
    : config_(config),
      feature_dim_(config.feat_type == "mfcc" ? config.num_ceps
                                               : config.num_bins),
      fbank_(config.num_bins, config.sample_rate, config.frame_length,
             config.frame_shift),
      mfcc_(config.num_ceps, config.num_bins, config.sample_rate,
